...
from GLAN_Model.myMetaLayer import MetaLayer
...
class EdgeModel(torch.nn.Module):
    def __init__(self, dim_edge, dim_node):
        ...
        self.edge_mlp1 = Seq(Lin(dim_edge + dim_node * 2, dim_edge * 2), ReLU(), GlobalNorm(),
                             Lin(dim_edge * 2, dim_edge), ReLU(), GlobalNorm())
        self.edge_mlp2 = Seq(Lin(dim_edge * 2, dim_edge * 2), ReLU(), GlobalNorm(), Lin(dim_edge * 2, dim_edge), ReLU(),
                             GlobalNorm())
...
class NodeModel(torch.nn.Module):
    def __init__(self, dim_edge, dim_node):
        ...
        self.node_mlp_1 = Seq(Lin(dim_edge + dim_node, dim_edge + dim_node), ReLU(), GlobalNorm(),
                              Lin(dim_edge + dim_node, dim_node), ReLU(), GlobalNorm())
        self.node_mlp_2 = Seq(Lin(dim_node * 2, dim_node * 2), ReLU(), GlobalNorm(), Lin(dim_node * 2, dim_node),
                              ReLU(), GlobalNorm())
...
class Model(torch.nn.Module):
    def __init__(self, layer_num, edge_dim, node_dim):
        ...
        for idx in range(self.layer_num):
            ...
            self.core_layers.add_module(coreName,
                                        MetaLayer(EdgeModel(edge_dim, node_dim), NodeModel(edge_dim, node_dim)))
